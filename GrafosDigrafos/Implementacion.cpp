...
// Reconstruye el camino (en forma de texto "1 -> 2 -> 3") desde el origen
// hasta destinoIdx, usando el arreglo de predecesores generado por Dijkstra.
static string construirCaminoTexto(int destinoIdx, vector<int>& predecesor, vector<int>& nodos) {
    vector<int> camino;
    ...
    // 'camino' queda en orden destino -> origen, hay que invertirlo
    string resultado = "";
...
void GrafoBase::calcularDijkstra(int origen, bool porCosto) {
    ...
    for (int i = 0; i < m; i++) idAIndice[nodos[i]] = i;
    ...
    for (size_t i = 0; i < rutas.size(); i++) {
        ...
        if (!esDirigido && peso < adj[v][u]) adj[v][u] = peso;
    }
    ...
    for (int i = 0; i < m; i++) {
        ...
        // Reconstruir camino usando IDs reales
        vector<int> camino;
        int cursor = idReal;
        while (cursor != -1) {
            camino.push_back(cursor);
            if (cursor == origen) break;
            if (cursor >= (int)ultPrev.size()) break;
            cursor = ultPrev[cursor];
        }
        for (int k = (int)camino.size() - 1; k >= 0; k--) {
            cout << camino[k];
            if (k > 0) cout << " -> ";
        }
        cout << "\n";
        ...
        hayDestinos = true;
        if (dist[i] < distMasCercano) { distMasCercano = dist[i]; idMasCercano = idReal; }
        if (dist[i] > distMasLejano)  { distMasLejano  = dist[i]; idMasLejano  = idReal; }
    }

    // Mostrar nodo origen al final
    cout << "  Nodo Origen " << origen << " -> Nodo Destino " << origen
         << " : 0 " << (porCosto ? "pesos" : "min") << "\n";
    cout << "     Camino: " << origen << "\n";
...
void GrafoBase::enumerarTodasLasRutas(int origen, int destino, bool porCosto) {
    ...
    map<int, vector<tuple<int,int,string>>> adyacencia;
set<pair<int,int>> aristasAgregadas;  // <-- necesita #include <set> arriba

for (size_t i = 0; i < rutas.size(); i++) {
    int u = rutas[i].nodoInicial;
    int v = rutas[i].nodoFinal;
    int peso = porCosto ? rutas[i].costo : rutas[i].tiempo;
    string nombre = rutas[i].aristaConexion;

    // Normalizar para no dirigido: siempre guardar el par (min, max)
    int clave1 = esDirigido ? u : min(u, v);
    int clave2 = esDirigido ? v : max(u, v);

    if (aristasAgregadas.count({clave1, clave2}) == 0) {
        aristasAgregadas.insert({clave1, clave2});
        adyacencia[u].push_back(make_tuple(v, peso, nombre));
        if (!esDirigido) {
            adyacencia[v].push_back(make_tuple(u, peso, nombre));
        }
    }
}
...
if (!hayResultadoDijkstra) {
    ...
    for (size_t i = 0; i < ultDistancias.size(); i++) {
        if (nodoExiste((int)i)) {
            ...
            // Reconstruir camino desde ultPrev
            vector<int> camino;
            int cursor = (int)i;
            while (cursor != -1 && cursor < (int)ultPrev.size()) {
                camino.push_back(cursor);
                cursor = ultPrev[cursor];
            }
            ...
            archivo << "      {\n";
            archivo << "        \"nodo\": " << i << ",\n";
            archivo << "        \"alcanzable\": " << (ultAlcanzable[i] ? "true" : "false") << ",\n";
            archivo << "        \"distancia\": " << (ultAlcanzable[i] ? ultDistancias[i] : 0) << ",\n";
            archivo << "        \"camino\": [";
            for (int k = (int)camino.size() - 1; k >= 0; k--) {
                archivo << camino[k];
                if (k > 0) archivo << ", ";
